...
import { Users, UserPlus, Trash2, Loader2, ArrowLeft } from "lucide-react";
...
export default function OrganizerTeamPage() {
  ...
  const { user, context, checked, canManageTeam, activeOrganization } = useAdminContext();
  ...
  useEffect(() => {
    ...
    if (!canManageTeam) { router.replace("/organizer/dashboard"); return; }
  }, [checked, user, canManageTeam, router]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([m, i]) => { setMembers(m); setInvitations(i.filter((inv) => !inv.accepted_at && !inv.revoked_at)); })
  ...
  async function handleAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    setAddError(null);
    try {
      const newMember = await apiFetch<OrgMember>(`/organizations/${orgId}/members`, {
        method: "POST",
        body: JSON.stringify({ email: addEmail.trim(), role: "organizer" }),
      });
    ...
    <div className="min-h-dvh bg-background">
      <header className="sticky top-0 z-40 border-b border-border bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center gap-3 px-4 py-3">
          <Link href="/organizer/dashboard" className="text-muted-foreground hover:text-foreground">
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <Users className="h-4 w-4 text-accent" />
          <span className="font-display text-base font-semibold">Team</span>
          {activeOrganization && (
            <span className="text-sm text-muted-foreground">· {activeOrganization.organization_name}</span>
          )}
        </div>
      </header>
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        ...
        <section className="mb-8">
          <h2 className="mb-4 font-display text-base font-semibold">Add organizer</h2>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            If the person doesn&apos;t have an account yet, they&apos;ll be invited — they get access as soon as they sign in.
          </p>
        </section>
        ...
        <section className="mb-6">
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-2">
              {members.map((m) => (
                <div key={m.user_id} className="flex items-center justify-between gap-4 rounded-2xl border border-border bg-card p-4">
                  <div>
        ...
        {invitations.length > 0 && (
          <section>
            ...
            <div className="flex flex-col gap-2">
              {invitations.map((inv) => (
                <div key={inv.id} className="flex items-center justify-between gap-4 rounded-2xl border border-dashed border-border bg-muted/30 p-4">
                  <div>
